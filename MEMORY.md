...
- **Resolve "ICP by Data Maturity" section in `strategy.md` (tagged TO BE DISCUSSED).** Three open questions: is Tier 2 sellable without existing analytics pain? Source third-party seed data during cold-start or not? Does systematic exploration need a distinct dashboard mode?
...
- 2026-06-15: Added Zone 4 column to `adaptive-ui-mapping.html` — matrix, legend, reading cards, and Data Availability table all now cover Zones 1–4. Added new "ICP by Data Maturity" section to `strategy.md` (tagged TO BE DISCUSSED): three tiers (Full / Cold-start / Not yet), plus explicit "what we do NOT do" — no DS-building, no analytics replacement, no serving customers without a DS.
- 2026-07-05: Made **within-customer cross-user cold-start** an explicit value prop. Tier 1 row in `strategy.md` now states that new users get adapted UI from their first session by inheriting profile shapes from the customer's existing users. `product-brief.md` Data Richness table's "New user" row updated to "Active signals + similar-user profiles derived from this product's existing users," with a new paragraph explaining the mechanism. This is distinct from the cross-*customer* network effect (already covered) — it's the intra-customer cross-user benefit that pitches directly to buyers' new-signup concern.
